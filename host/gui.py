...
class ControllerGUI:
    ...
    def refresh_recording_controls(self):
        if self.data_logger.active:
            ...
            self.recording_label.configure(
                text=(
                    f"{filename} | "
                    f"{self.data_logger.sample_count} telemetry samples"
                ),
                fg="#1b5e20",
            )
        else:
    ...
    def toggle_recording(self):
        ...
        self.set_status(f"Recording commands and battery data to {destination}")
        self.refresh_recording_controls()
    ...
    def stop_recording(self, event="recording_stopped"):
        ...
        try:
            ...
            self.clear_fault("recording")
            self.set_status(
                f"Saved {samples} telemetry samples to {destination}", "green"
            )
