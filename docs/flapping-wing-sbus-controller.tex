...
slots, one 32-bit sequence, and one 8-bit host-failsafe flag, for a total of
37 bytes. A new sequence is assigned to every accepted PC command and
transmitter-side failsafe change. It is sent immediately after a valid command;
50~ms wireless heartbeat retransmissions keep the same sequence.
...
interface, not the PC serial format. Both copies of
\file{wireless/*/esp_now_link.h} must remain binary compatible and both MCUs
must be reflashed after a payload-format change.
...
Transmitter to receiver & 37-byte binary ESP-NOW packet & 50 ms heartbeat \\
...
31-byte layout is shown below; multibyte fields use the ESP32's native
...
\code{last_sequence} & uint32 & 4 & Latest accepted forward sequence \\
\code{applied_ch[6]} & uint16[6] & 12 & Confirmed CH1, CH2, CH3, CH5, CH6, CH8 host values \\
\code{battery_adc_raw} & uint16 & 2 & Averaged raw ADC count \\
\code{battery_pin_mv} & uint16 & 2 & Averaged GPIO3 millivolts \\
\code{version} & uint8 & 1 & Receiver-status version, currently 3 \\
...
The sequence and channel fields confirm the latest forward packet accepted and
mapped by the receiver. They represent values placed on the SBUS wire only
while \code{link=1} and \code{failsafe=0}. Because telemetry is produced at
4~Hz while PC commands normally arrive at 10~Hz, intermediate commands can be
applied without appearing in a separate status line.
...
Battery voltage
  ...
  -> Python ser.readline()
  -> parse confirmed sequence and channels
...
\subsection{Live receiver-confirmed CSV recording}

Reusable parsing and recording functions live in \file{wireless/pc_tools/}.
...
Each CSV status row contains PC time, commanded and receiver-confirmed channels,
the confirmed sequence, a command/confirmation comparison, link/failsafe
state, packet count, raw ADC, GPIO3 millivolts, and reconstructed battery volts.
Rows are flushed immediately. The GUI and standalone recorder cannot own the
same COM port simultaneously.
...
\midrule
\code{IDENTIFY\n} & \code{DEVICE:} identity line & Verify firmware, protocol 3, and radio initialization \\
...
received, not as replies to a particular PC command. The \code{sequence} and
channel fields confirm the receiver's latest accepted state.
...
receiver telemetry, displays receiver-confirmed channels and reconstructed
battery voltage, locks CH8 on detected faults, and requires confirmation for
arming and benchmark start. Its \textbf{START RECORDING} button writes the live
data to CSV. It implements the same serial protocol described in Section~4.
...
wireless/gui.py                GUI, benchmark, and CSV recording entry point
...
\code{RECEIVER_VALUE_TEST} in \file{wireless/receiver/receiver.ino} is disabled
...
\begin{itemize}
  ...
  \item 4~Hz telemetry confirms the latest state but can omit intermediate
        10~Hz PC commands;
...
Expected identity & \code{DEVICE:FLAPPING_WING_TRANSMITTER;PROTOCOL:3;RADIO:1} \\
...
ESP-NOW control & Channel 1, 37-byte packed packet, 50~ms heartbeat \\
SBUS output & GPIO4, inverted, 100000 baud 8E2, approximately 10~ms \\
Battery input & GPIO3 through 300/100~k$\Omega$ divider \\
Telemetry & 31-byte status with confirmed sequence/channels every 250~ms \\
