...
2. send `IDENTIFY\n` until it receives the expected protocol-3 identity;
...
5. parse `RECEIVER_STATUS ...` lines for the receiver-confirmed sequence,
   channels, link, failsafe, packet count, and battery data; and
...
```text
PONG;PROTOCOL:3;HOST_FAILSAFE:0;RADIO:1;UPTIME_MS:12345
```
...
| `sequence` | Sequence of the latest forward state accepted and mapped by the receiver |
| `link` | `1` only while receiver SBUS output is actually active |
| `failsafe` | `0` only while the receiver considers control ready |
| `ch1`, `ch2`, `ch3`, `ch5`, `ch6`, `ch8` | Latest receiver-confirmed host-unit values |
...
The confirmed channel fields mean that the receiver accepted the ESP-NOW
packet and mapped those host values. They mean the values are on the SBUS wire
only when `link=1` and `failsafe=0`. Because status is 4 Hz while PC commands
are normally 10 Hz, intermediate commands can be applied without appearing in
...
delays the lock. When the connection remains available, wait for a fresh
`RECEIVER_STATUS` with `ch8=1000` to confirm that the receiver accepted the
locked value.
...
- PC ASCII channel commands have no checksum or explicit malformed-command
  response.
- The transmitter assigns the sequence after accepting a PC command, and the
  sequence restarts when the transmitter reboots.
- Receiver telemetry is 4 Hz, so it confirms the latest applied state rather
  than acknowledging every 10 Hz PC heartbeat individually.
